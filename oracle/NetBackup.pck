...
--------
  ...
  begin 
    ...
    insert into host
      (host_type, os, os_type, version, status, name, ip, date_insert, date_update)
    values
      (pHostType, pOS, pOSType, pVersion, pStatus, pName, pIP, GetDateUpdate, GetDateUpdate)
    returning host_id into HostID;
    commit;
    return HostID;
  end  SetHost;           
  ...
  begin 
    ...
    insert into policy
      (name, type, job_priority, max_jobs, active, active_start, 
       ...
       use_vm, date_insert, date_update)
    values
      (pPolicyName, pType, pJobPriority, pMaxJobs, pActive, pStarted, 
       ...
       pUseVM, GetDateUpdate, GetDateUpdate)
    returning policy_id into PolicyID;
...
--    sql_rowcount := SQL%ROWCOUNT;
    ...
    insert into job
      (id, type, state, status, start_time, end_time, active_start, active_elapsed, 
...
--------
  ...
  begin 
    insert into jobdetail
      (id, try, nline, linestr, date_insert, date_update)
    values
      (pJobID, pTry, pNLine, pLineStr, GetDateUpdate, GetDateUpdate);
